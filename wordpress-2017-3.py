...
def get_posts(api_base):
    ...
    for post in posts:
        print('-Post ID: %s, Title: %s, Url: %s' % (post['id'], post['title']['rendered'], post['link']))
...
if __name__ == '__main__':
    ...
    print('* Updating post %s' % sys.argv[2])
    with open(sys.argv[3], 'r') as content:
        new_content = content.readlines()

    #update_post(api_url, sys.argv[2], ''.join("123"))
